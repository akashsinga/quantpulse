...
try:
    ...
    database_url = os.getenv(
        "DATABASE_URL", "postgresql://postgres:password@localhost/quantpulse")
...
try:
    ...
    logger.warning(
        "WARNING: Could not import models. Running with empty metadata.")
...
def run_migrations_online() -> None:
    ...
    connectable = engine_from_config(section,
                                     prefix="sqlalchemy.",
                    ...
                    if result:
                        ...
                        for table_name in [
                            ...
                            if table_exists and not is_hypertable(table_name):
                                ...
                                logger.info(
                                    f"Created hypertable for {table_name}")
            except Exception as e:
                logger.warning(
                    f"Warning: Failed to set up TimescaleDB hypertables: {e}")
