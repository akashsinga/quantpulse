...
class Settings(BaseSettings):
    ...
    # OHLCV Fetcher Configuration - Optimized for performance
    OHLCV_WORKERS: int = int(os.getenv("OHLCV_WORKERS", "20"))  # Increased for parallel processing
    OHLCV_BATCH_SIZE: int = int(os.getenv("OHLCV_BATCH_SIZE", "100"))  # Optimized batch size
    ...
    OHLCV_BULK_INSERT_SIZE: int = int(os.getenv("OHLCV_BULK_INSERT_SIZE", "1000"))  # Optimized for PostgreSQL
    ...
    OHLCV_BATCH_SIZE_TODAY: int = int(os.getenv("OHLCV_BATCH_SIZE_TODAY", "200"))
    OHLCV_HISTORICAL_WORKERS: int = int(os.getenv("OHLCV_HISTORICAL_WORKERS", "6"))  # Optimized concurrent fetches

    # Weekly Data Aggregation Settings - NEW OPTIMIZED SETTINGS
    WEEKLY_AGGREGATION_BATCH_SIZE: int = int(os.getenv("WEEKLY_AGGREGATION_BATCH_SIZE", "200"))  # Securities per batch
    WEEKLY_AGGREGATION_MAX_WORKERS: int = int(os.getenv("WEEKLY_AGGREGATION_MAX_WORKERS", "6"))  # Parallel workers
    WEEKLY_AGGREGATION_CHUNK_SIZE: int = int(os.getenv("WEEKLY_AGGREGATION_CHUNK_SIZE", "1000"))  # Records per DB chunk
    ...
    # TimescaleDB Optimization Settings - NEW
    ...
    PARALLEL_BATCH_PROCESSING: bool = os.getenv("PARALLEL_BATCH_PROCESSING", "True").lower() == "true"
    MAX_PARALLEL_BATCHES: int = int(os.getenv("MAX_PARALLEL_BATCHES", "4"))  # Celery worker batches
    SECURITIES_PER_PARALLEL_BATCH: int = int(os.getenv("SECURITIES_PER_PARALLEL_BATCH", "500"))
    ...
    # Database pool settings - Enhanced for high-performance aggregation
    DB_POOL_SIZE: int = int(os.getenv("DB_POOL_SIZE", "100"))  # Increased for parallel processing
    DB_MAX_OVERFLOW: int = int(os.getenv("DB_MAX_OVERFLOW", "100"))  # Increased overflow
    ...
    DB_CONNECT_ARGS: dict = {
        ...
        "server_settings": {
            "application_name": "quantpulse_optimized",
            # PostgreSQL optimization settings for time-series data
            "shared_preload_libraries": "timescaledb",
            "max_connections": "200",
            "work_mem": "256MB",  # Increased for aggregation queries
            "maintenance_work_mem": "1GB",  # For index creation
            "effective_cache_size": "4GB",  # Adjust based on system RAM
            "random_page_cost": "1.1",  # For SSD storage
        },
    }

    # Query Optimization Settings - NEW
    ...
    # Redis Configuration (for Celery and caching) - Enhanced for parallel processing
    REDIS_URL: str = os.getenv("REDIS_URL", "redis://localhost:6379/0")
    REDIS_MAX_CONNECTIONS: int = int(os.getenv("REDIS_MAX_CONNECTIONS", "100"))  # Increased for parallel tasks
    REDIS_CONNECTION_POOL_SIZE: int = int(os.getenv("REDIS_CONNECTION_POOL_SIZE", "50"))

    # Celery Configuration - Optimized for parallel processing
    ...
    CELERY_WORKER_CONCURRENCY: int = int(os.getenv("CELERY_WORKER_CONCURRENCY", "16"))  # Parallel workers
    CELERY_WORKER_PREFETCH_MULTIPLIER: int = int(os.getenv("CELERY_WORKER_PREFETCH_MULTIPLIER", "4"))
    CELERY_TASK_SOFT_TIME_LIMIT: int = int(os.getenv("CELERY_TASK_SOFT_TIME_LIMIT", "3600"))  # 1 hour
    CELERY_TASK_TIME_LIMIT: int = int(os.getenv("CELERY_TASK_TIME_LIMIT", "7200"))  # 2 hours
    ...
    MAX_MEMORY_USAGE_MB: int = int(os.getenv("MAX_MEMORY_USAGE_MB", "4096"))  # 4GB
