...
class Security(Base):
    ...
    __table_args__ = (
        UniqueConstraint("symbol", "exchange_id", name="uq_symbol_exchange"),
        CheckConstraint(
            "(security_type IN ('STOCK', 'INDEX') AND futures_id IS NULL) OR (security_type = 'DERIVATIVE')",
            name="chk_security_type_consistency"))

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    symbol = Column(String(100), nullable=False, index=True)
    name = Column(String(255), nullable=False)
    exchange_id = Column(UUID(as_uuid=True),
                         ForeignKey("exchanges.id"),
                         nullable=False)
    security_type = Column(String(20),
                           nullable=False)  # STOCK, INDEX, DERIVATIVE
    segment = Column(String(20), nullable=False)  # EQUITY, CURRENCY, COMMODITY
    external_id = Column(Integer, nullable=False,
                         index=True)  # Dhan's Security ID
    ...
    strategy_securities = relationship("StrategySecurity",
    ...
    futures = relationship("Future",
                           back_populates="security",
                           uselist=False,
                           foreign_keys="Future.security_id")

    # Fixed: Add overlaps parameter to resolve SQLAlchemy warning
    derivative_underlyings = relationship(
        "Security",
        secondary="futures",
        primaryjoin="Security.id==Future.security_id",
        secondaryjoin="Security.id==Future.underlying_id",
        backref="derivatives",
        overlaps="futures")
