...
async def start_securities_import(
    ...
    """
    logger.info(
        f"Securities import requested by admin user {current_user.email}")
    ...
    try:
        ...
        response = ImportResponse(
            task_id=task.id,
            status="PENDING",
            message="Securities import started successfully",
...
async def get_import_status(
    task_id: str, current_user: User = Depends(get_current_superadmin)):
    """
    ...
    try:
        ...
        if task_result.state == 'PENDING':
            ...
            response = TaskStatusResponse(task_id=task_id,
                                          status='PROGRESS',
